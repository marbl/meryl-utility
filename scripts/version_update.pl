...
if (($modName eq "") || ($modName eq undef)) {
...
open(F, "> version.H.new") or die "Failed to open 'version.H.new' for writing: $!\n";
...
if (compare("version.H", "version.H.new") == 0) {
    unlink "version.H.new";
} else {
    unlink "version.H";
    rename "version.H.new", "version.H";
}
