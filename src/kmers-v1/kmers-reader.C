...
merylutil::kmers::v1::merylFileReader::initializeFromMasterIndex(bool  doInitialize,
  ...
  if (merylutil::fileExists(N) == false)
...
merylutil::kmers::v1::merylFileReader::loadBlockIndex(void) {
  ...
  for (uint32 ii=0; ii<_numFiles; ii++) {
    ...
    merylutil::loadFromFile(_blockIndex + _numBlocks * ii, "merylFileReader::blockIndex", _numBlocks, idxfile);
...
dumpMerylDataFile(char *name) {
  ...
  using namespace   merylutil::bits;
  ...
  if (merylutil::fileExists(name, '.', "merylIndex") == false)
  ...
  while (merylutil::loadFromFile(I, "merylFileIndex", F, false) != 0) {
  ...
  if (merylutil::fileExists(name, '.', "merylData") == false)
